...
class Save extends \Magestore\Bannerslider\Controller\Adminhtml\Banner
{
    ...
    public function execute()
    {
        ...
        if ($data = $this->getRequest()->getPostValue()) {
            ...
            $imageRequest = $this->getRequest()->getFiles('image_paid');
            if ($imageRequest) {
                if (isset($imageRequest['name'])) {
                    $fileName = $imageRequest['name'];
                } else {
            ...
            if ($imageRequest && strlen($fileName)) {
                 ...
                 */
                try {
                    $uploader = $this->_uploaderFactory->create(['fileId' => 'image_paid']);
                    ...
                    $data['image_paid'] = \Magestore\Bannerslider\Model\Banner::BASE_MEDIA_PATH.$result['file'];
                } catch (\Exception $e) {
                    if ($e->getCode() == 0) {
                        $this->messageManager->addError($e->getMessage());
                    }
                ...
                if (isset($data['image_paid']) && isset($data['image_paid']['value'])) {
                    if (isset($data['image_paid']['delete'])) {
                        $data['image_paid'] = null;
                        $data['delete_image'] = true;
                    } elseif (isset($data['image_paid']['value'])) {
                        $data['image_paid'] = $data['image_paid']['value'];
                    } else {
                        $data['image_paid'] = null;
                    }
                }
            }
            $imageRequest = $this->getRequest()->getFiles('image_mobile');
            if ($imageRequest) {
                if (isset($imageRequest['name'])) {
                    $fileName = $imageRequest['name'];
                } else {
            ...
            if ($imageRequest && strlen($fileName)) {
                 ...
                 */
                try {
                    $uploader = $this->_uploaderFactory->create(['fileId' => 'image_mobile']);
                    ...
                    $data['image_mobile'] = \Magestore\Bannerslider\Model\Banner::BASE_MEDIA_PATH.$result['file'];
                } catch (\Exception $e) {
                    if ($e->getCode() == 0) {
                        $this->messageManager->addError($e->getMessage());
                    }

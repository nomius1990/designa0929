...
module.exports = function() { // eslint-disable-line func-names
  ...
  const plugins              = this.opts.plugins,
        ...
        configPath           = config.projectPath + 'dev/tools/gulp/config/';
  ...
  // Copy all all non existent config files to /dev/tools/gulp/config/
  plugins.fs.readdirSync(configSamplesPath).forEach((fileName) => {
    ...
    try {
      ...
      plugins.util.log('File ' + fileName + ' copied to /dev/tools/gulp/config/' + newFileName);
    }
    catch (error) {
      plugins.util.log(
  ...
  plugins.util.log(
    plugins.util.colors.green('Setup complete! Go to "/dev/tools/gulp/config/" directory and edit the configuration there.')
  );
};

...
module.exports = function(resolve) { // eslint-disable-line func-names
  ...
  Object.defineProperty(plugins, 'path', require('path'));
  const path = require('path');
  const babel = require('../helper/babel');
  const cssLint = require('../helper/css-lint');
  const dependecyTree = require('../helper/dependency-tree-builder');
  const inheritance = require('../helper/inheritance-resolver');
  const sass = require('../helper/scss');
  const sassLint = require('../helper/sass-lint');
  const svg = require('../helper/svg');

  plugins.helper = {};
  plugins.helper.babel = babel;
  plugins.helper.cssLint = cssLint;
  plugins.helper.dependecyTree = dependecyTree;
  plugins.helper.inheritance = inheritance;
  plugins.helper.sass = sass;
  plugins.helper.sassLint = sassLint;
  plugins.helper.svg = svg;
  ...
  themes.forEach(name => {
    ...
    tempWatcher.on('change', path => {
      ...
      // Babel
      if (plugins.path.basename(path).includes('.es6.js')) {
